...
// Job is scheduled for UPDATING TOTAL WORLD CASES When we want use cron scheduler
async function update_total_world_cases() {
...
async function update_total_india_cases() {
  try {
...
async function update_state_data_india() {
  try {
    axios
      .delete(process.env.DELETE_STATE_DATA)
      .then((response) =>
        console.log("Length of state data deleted:", response.data)
      );
    const Response = await axios.get(process.env.API_TOTAL_INDIA);
    var Data = Response.data;
...
async function update_individual_country_data() {
  try {
    axios
      .delete(process.env.DELETE_COUNTRY_DATA)
      .then((response) =>
        console.log("Length of country data deleted:", response.data)
      );
...
async function load() {
  ...
  while (1) {
    update_total_world_cases();
    update_total_india_cases();
    update_individual_country_data();
    update_state_data_india();
    console.log("Database updated at", Date());
    await timer(72000000); // then the created Promise can be awaited ie 2hrs over here
  }
}

module.exports = load();

// (cron.schedule("57 13 * * *", async(req,res)=>{}) is to schedule cron jobs without using this while loop but it doesn't work on cloud platform we need to pay for schedulers that is why workaround of infinite loop is used with time delay.
